...
app.route('/get_attraction')
    .get(function(req, res) {
        ...
        var uurl = serverName + "";
        if (category === "City")
            uurl += "attractionc/cityId=" + id;
        else if (category === "Museum")
            uurl += "attractionm/museumId=" + id;
...
app.post('/compute-plan-city', function(req, res) {
    ...
    uname = uname.toLowerCase().replace(/ /g, "_");
    var uid = req.body.id; //1 = roma
    var visits = req.body.number_visits;
    logger.info("Number of visits:", visits);
    var must = req.body.must; //must
    logger.debug("must: ", JSON.stringify(must));
    for(var i = 0; i < must.must.length; i++) {
        must.must[i].name = must.must[i].name.toLowerCase().replace(/ /g, "_");
    }
    var exclude = req.body.exclude; //exclude
    logger.debug("exclude: ", JSON.stringify(exclude));
    for(var i = 0; i < exclude.exclude.length; i++) {
        exclude.exclude[i].name = exclude.exclude[i].name.toLowerCase().replace(/ /g, "_");
    }
    ...
    request({
        ...
        if (!error & response.statusCode === 200) {
            logger.debug("attractionsc: 200 OK");
            var attractions = [];
            for (var i = 0; i < body.length; i++) {
                attractions.push(body[i].name.toLowerCase().replace(/ /g, "_"));
                attractionsArray[body[i].id] = body[i];
            }
            //
            //Remove attractions from states list
            //TODO
            for(var i = 0; i  < exclude.exclude.length; i++) {
                for(var j = 0; j < attractions.length; j++) {
                    if(attractions[j] === exclude.exclude[i])
            ...
            async.series([
                ...
                function(callback) {
                    ...
                    request({
                        ...
                        if (!error & response.statusCode === 200) {
                            ...
                            //logger.debug(pathName + "TCODA body.sensing:\n", JSON.stringify(body));
                            if (body.length > 1) {
                                /*
                                for (var i = 0; i < body.length; i++) {
                                    var name = body[i].TQueue.attractionC.name.toLowerCase().replace(/ /g, "_");
                                ...
                                if (!error & response.statusCode === 200) {
                                    ...
                                    if (body.length > 1) {
                                        ...
                                        for (var i = 0; i < body.length; i++) {
                                            ...
                                            var cost = attractionsTimeMap[id];
                                            for (var j = 0; j < visits; j++) {
                ...
                function(callback) {
                    ...
                    //Gather time data between attractions for the domain
                    function distanceBetweenAttractions(sorg, sorgId, dest, destId, sorgLat, sorgLong, destLat, destLong) {
                        var apiKey = ' AIzaSyD0xbJJwC7pQBzNFupb2s7orzOvB_ctSb4 ';
                        reqUrl = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" + sorgLat + "," + sorgLong;
                        reqUrl += "&destinations=" + destLat + "," + destLong;
                        ...
                        request({
                            ...
                            if (!error & response.statusCode === 200) {
                                if (body.status === 'OK') {
                                    ...
                                    var src = sorg.toLowerCase().replace(/ /g, "_");
                                    var des = dest.toLowerCase().replace(/ /g, "_");
                                    var string;
                                    string = "(:action move-" + src + "-" + des + "\n\t\t";
                                    string += ":precondition (cur_state top_" + src + ")\n\t\t";
                                    string += ":effect (and (cur_state top_" + des + ") (not(cur_state top_" + src + ")) ";
                    ...
                    function distanceFromStart(lat, long, dest, latDest, longDest, umail) {
                        var apiKey = ' AIzaSyD0xbJJwC7pQBzNFupb2s7orzOvB_ctSb4 ';
                        var reqUrl = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" + lat + "," + long;
                        reqUrl += "&destinations=" + latDest + "," + longDest;
                        reqUrl += "&language=it-IT&mode=" + tr_mode + "&key=" + apiKey;
                        request({
                            url: reqUrl,
                            method: "POST",
                            ...
                            if (!error & response.statusCode === 200) {
                                if (body.status === 'OK') {
                                    ...
                                    var des = dest.toLowerCase().replace(/ /g, "_");
                                    var string;
                                    string = "(:action move-start-" + des + "\n\t\t";
                                    string += ":precondition (cur_state start)\n\t\t";
                                    string += ":effect (and (cur_state top_" + des + ") (not(cur_state start)) ";
                                    string += "(increase (total-cost) " + minutes + "))\n\t)\n\t";
                                    fs.appendFileSync(domainFile,string,'utf8');
                                }
                            }
...
app.post('/report_queue', function(req, res) {
    ...
    if (type === 'Queue') {
        ...
        if (category === "Museum") {
            request({
                url: serverName + "tcoda/museum/" + attrId + "," + minutes,
                method: "GET",
                json: true,
                headers: [{
                    'content-type': 'application/json'
    ...
    if (type === 'Visit') {
        ...
        if (category === 'Museum') {
            request({
                url: serverName + "tvisita/museum/" + attrId + "," + minutes,
                method: "GET",
                json: true,
                headers: [{
                    'content-type': 'application/json'
            ...
            */
            ...
            /*var urlCoda = serverName + "sensing/queue/museum/museumId=" + uid;
                    request({
                        ...
                        logger.debug("tcoda body: ", JSON.stringify(body));
                    });*/
                            ...
                            request({
                                ...
                                if (!error /*&& response.statusCode === 200*/) {
                                    ...
                                    //if (body.length > 1) {
                                        ...
                                        //for (var i = 0; i < body.length; i++) {
                                        for (var i = 0; i < attractions.length; i++) {
                                            //var name = body[i].TVisit.attractionM.name.toLowerCase().replace(/ /g, "_");
                                            var name = attractions[i].id;
                                            //attractionsTimeMap[name] += +(body[i].TVisit.minutes);
                                            //var cost = attractionsTimeMap[name];
                                            var cost = 1;
                                            for (var j = 0; j < visits; j++) {
                                                string += "(:action visit-v" + j + "-" + name + "\n\t\t";
                                                string += ":precondition (and (cur_state " + attr2room[name] + ") (cur_state v" + j + ") (not (visited " + name + ")))\n\t\t";
                                                string += ":effect (and (cur_state v" + (j + 1) + ") (not (cur_state v" + j + ")) (visited " + name +") (increase (total-cost) " + cost +"))\n\t)\n\t";
                                            }
                                        }
                                        fs.appendFileSync(domainFile, string, 'utf-8');
                                        callback();
